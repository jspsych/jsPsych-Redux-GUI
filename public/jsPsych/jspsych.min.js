...
b.readOnly=function(){return c(jsPsych.utils.deepCopy(e))};b.addToAll=function(a){for(var d=0;d<e.length;d++)for(var f in a)e[d][f]=a[f];return b};b.addToLast=function(a){if(0!=e.length)for(var d in a)e[e.length-1][d]=a[d];return b};b.filter=function(a){a=Array.isArray(a)?jsPsych.utils.deepCopy(a):jsPsych.utils.deepCopy([a]);for(var b=[],d=0;d<e.length;d++){for(var f=!1,g=0;g<a.length;g++){for(var k=!0,m=Object.keys(a[g]),h=0;h<m.length;h++)if("undefined"===typeof e[d][m[h]]||e[d][m[h]]!=a[g][m[h]])k=
!1;if(k){f=!0;break}}f&&b.push(e[d])}return c(b)};b.filterCustom=function(a){for(var b=[],d=0;d<e.length;d++)a(e[d])&&b.push(e[d]);return c(b)};b.select=function(a){for(var b=[],f=0;f<e.length;f++)"undefined"!==typeof e[f][a]&&b.push(e[f][a]);a=d();a.values=b;return a};b.ignore=function(a){Array.isArray(a)||(a=[a]);for(var b=jsPsych.utils.deepCopy(e),d=0;d<b.length;d++)for(var f in a)delete b[d][a[f]];return c(b)};b.uniqueNames=function(){for(var a=[],b=0;b<e.length;b++)for(var d=Object.keys(e[b]),
f=0;f<d.length;f++)a.includes(d[f])||a.push(d[f]);return a};b.csv=function(){var a=e;a="object"!=typeof a?JSON.parse(a):a;for(var b="",d="",f=[],c=0,g=0;g<a.length;g++)for(var k in a[g]){var h=k+"";h='"'+h.replace(/"/g,'""')+'",';f.includes(k)||(f[c]=k,b+=h,c++)}b=b.slice(0,-1);d+=b+"\r\n";for(c=0;c<a.length;c++){b="";for(g=0;g<f.length;g++)b+='"'+(("undefined"===typeof a[c][f[g]]?"":a[c][f[g]])+"").replace(/"/g,'""')+'",';b=b.slice(0,-1);d+=b+"\r\n"}return d};b.json=function(a){return a?JSON.stringify(e,
...
a.preamble+"</div>");for(b=0;b<a.questions.length;b++){c+='<div id="jspsych-survey-text-"'+b+'" class="jspsych-survey-text-question" style="margin: 2em 0em;">';c+='<p class="jspsych-survey-text">'+a.questions[b].prompt+"</p>";var e=0==b?"autofocus":"";c=1==a.questions[b].rows?c+('<input type="text" name="#jspsych-survey-text-response-'+b+'" size="'+a.questions[b].columns+'" value="'+a.questions[b].value+'" '+e+"></input>"):c+('<textarea name="#jspsych-survey-text-response-'+b+'" cols="'+a.questions[b].columns+
'" rows="'+a.questions[b].rows+'" '+e+">"+a.questions[b].value+"</textarea>");c+="</div>"}c+='<button id="jspsych-survey-text-next" class="jspsych-btn jspsych-survey-text">'+a.button_label+"</button>";d.innerHTML=c;d.querySelector("#jspsych-survey-text-next").addEventListener("click",function(){for(var a=performance.now()-g,b={},c=d.querySelectorAll("div.jspsych-survey-text-question"),e=0;e<c.length;e++){var f="Q"+e,p=c[e].querySelector("textarea, input").value,r={};r[f]=p;Object.assign(b,r)}a={rt:a,
responses:JSON.stringify(b)};d.innerHTML="";jsPsych.finishTrial(a)});var g=performance.now()};return c}();jsPsych.plugins.video=function(){var c={};jsPsych.pluginAPI.registerPreload("video","sources","video");c.info={name:"video",description:"",parameters:{sources:{type:jsPsych.plugins.parameterType.VIDEO,pretty_name:"Sources",array:!0,default:void 0,description:"The video file to play."},width:{type:jsPsych.plugins.parameterType.INT,pretty_name:"Width",default:"",description:"The width of the video in pixels."},height:{type:jsPsych.plugins.parameterType.INT,pretty_name:"Height",default:"",description:"The height of the video display in pixels."},
